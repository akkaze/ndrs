...
use ndrs::{ArcTensorView, broadcast_shapes, Tensor, TensorViewOps};
use pyo3::prelude::*;
...
pub fn tensor_add_impl(a: &ArcTensorView, b: &ArcTensorView) -> PyResult<PyTensor> {
    let target_shape = broadcast_shapes(a.shape(), b.shape())
        .ok_or_else(|| pyo3::exceptions::PyValueError::new_err("Incompatible shapes for broadcast"))?;
    let a_bcast = a.broadcast_to(&target_shape)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e))?;
    let b_bcast = b.broadcast_to(&target_shape)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e))?;
